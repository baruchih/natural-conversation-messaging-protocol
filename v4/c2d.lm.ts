/**
 * C2-D one run. Generate, then score. Do not regenerate after a miss.
 */
...
import { A_INTENT, BOOTSTRAP_HINT, CONTROL_SEED, K, LAST, hasHint, isProbeC2D, pSec, tSec } from './c2d.ts';
...
const raw = await complete(intentPrompt([{ speaker: 'B', utterance: LAST }], 'A', A_INTENT, K));
const considered = parseCandidates(raw).slice(0, K);
const legalSet = considered.filter(legal);
const hits = legalSet.filter((u) => isProbeC2D(u));
const report = {
  model: openaiModel(),
  k: K,
  hint: BOOTSTRAP_HINT,
  ...
  candidates: considered.map((u) => ({
    ...
    legal: legal(u),
    p_sec: pSec(u),
    hit: legal(u) && isProbeC2D(u),
  })),
};
writeFileSync(resolve(import.meta.dirname, 'c2d.frozen.json'), JSON.stringify(report, null, 2) + '\n');
console.log(
  JSON.stringify(
    { considered: report.considered, with_hint: report.with_hint, legal: report.legal, hits: report.hits, hit: report.hit },
    null,
    2,
  ),
);
